...
int PlanManagerImpl::inst_ss_gen (Physical::Operator *op)
{
	...
	for (top = usedOps ; top ; top = top -> next) {
		...
		ASSERT (top -> instOp);
		// Nikos: I added the cast below to Monitor::PropertyMonitor*
		if ((rc = ssgen -> addOpEntity (top -> id, (Monitor::PropertyMonitor*) top -> instOp)) != 0)
			return rc;
		...
		if (top -> kind == PO_JOIN ||
			...
			// Nikos: I added the cast below to Monitor::PropertyMonitor*
			rc = ssgen -> addJoinEntity (top -> id, (Monitor::PropertyMonitor*) top -> instOp);
			if (rc != 0) return rc;
		}
	}
	...
	for (unsigned int q = 0 ; q < numQueues ; q++) {
		...
		// Nikos: I added the cast below to Monitor::PropertyMonitor*
		if ((rc = ssgen -> addQueueEntity (q, (Monitor::PropertyMonitor*)queues [q].instQueue)) != 0)
			return rc;
	}
	...
	for (unsigned int s = 0 ; s < numSyns ; s++) {
		...
		ASSERT (syns[s].u.relSyn);
				// Nikos: I added the cast below to Monitor::PropertyMonitor*
			if ((rc = ssgen -> addSynEntity (s, (Monitor::PropertyMonitor*)syns[s].u.relSyn)) != 0)
				return rc;
			break;
		...
		case PARTN_WIN_SYN:
			ASSERT (syns[s].u.pwinSyn);
				// Nikos: I added the cast below to Monitor::PropertyMonitor*
			if ((rc = ssgen -> addSynEntity (s, (Monitor::PropertyMonitor*)syns[s].u.pwinSyn)) != 0)
				return rc;
			break;
	...
	for (unsigned int s = 0 ; s < numStores ; s++) {
		...
		// Nikos: I added the cast below to Monitor::PropertyMonitor*
		if ((rc = ssgen -> addStoreEntity (s, (Monitor::PropertyMonitor*)stores[s].instStore)) != 0)
			return rc;
	}
									...
									unsigned int l_numSyns)
{
	...
	for (unsigned int o = 0 ; o < numOps ; o++) {
		...
		ASSERT (top -> instOp);
        // Nikos: I added the cast below to Monitor::PropertyMonitor*
		if ((rc = e_op -> addOpEntity (top -> id, (Monitor::PropertyMonitor*)top -> instOp)) != 0)
			return rc;
		...
		if (top -> kind == PO_JOIN ||
			...
			top -> kind == PO_STR_JOIN_PROJECT) {
            // Nikos: I added the cast below to Monitor::PropertyMonitor*
			rc = e_op -> addJoinEntity (top -> id, (Monitor::PropertyMonitor*)top -> instOp);
			if (rc != 0) return rc;
		}
	}
	...
	for (unsigned int q = 0 ; q < l_numQueues ; q++) {
		...
		if ((rc = e_op -> addQueueEntity (queueList[q] -> id,
                                          (Monitor::PropertyMonitor*)queueList[q] -> instQueue)) != 0)
			return rc;
	}
	...
	for (unsigned int s = 0 ; s < l_numSyns ; s++) {
		...
		case REL_SYN:
			...
			if ((rc = e_op -> addSynEntity (synList[s] -> id,
                                            (Monitor::PropertyMonitor*)synList[s] -> u.relSyn)) != 0)
				return rc;
			break;
		...
		case PARTN_WIN_SYN:
			ASSERT (synList[s] -> u.pwinSyn);
                // Nikos: I added the cast below to Monitor::PropertyMonitor*
			if ((rc = e_op -> addSynEntity (synList[s] -> id,
                                            (Monitor::PropertyMonitor*)synList[s] -> u.pwinSyn)) != 0)
				return rc;
			break;
	...
	for (unsigned int s = 0 ; s < l_numStores ; s++) {
		...
		if ((rc = e_op -> addStoreEntity (storeList[s] -> id,
                                          (Monitor::PropertyMonitor*)storeList[s] -> instStore)) != 0)
			return rc;
	}
